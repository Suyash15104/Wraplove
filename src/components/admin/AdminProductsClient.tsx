  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        formData.append('upload_preset', 'wraplove_products')
        const res = await fetch(`https://api.cloudinary.com/v1_1/dopdc66nb/image/upload`, {
          method: 'POST',
          body: formData,
        })
                    ...
                    <div className="flex items-center gap-3">
                      {p.images?.[0] ? (
                        <img src={p.images[0]} alt={p.name}
                          className="w-9 h-9 rounded-xl object-cover flex-shrink-0 border border-beige-200" />
                      ) : (
            ...
            >
              <div className="bg-white rounded-4xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6 space-y-4">
                  ...
                  {/* ── Image Upload ── */}
                  <div>
                    <label className="text-xs font-medium text-brand-muted block mb-2">Product Images</label>
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {images.map((url, i) => (
                          <div key={i} className="relative group w-20 h-20">
                            <img src={url} alt=""
                              className="w-20 h-20 rounded-xl object-cover border-2 border-beige-200" />
                            <button
                            ...
                            {i === 0 && (
                              <span className="absolute bottom-1 left-1 text-[9px] bg-brand-dark/70 text-white px-1 rounded">
                                Main
                              </span>
                            )}
                          </div>
                    ...
                    <button
                      ...
                      disabled={uploading}
                      className={cn(
                        'w-full border-2 border-dashed border-beige-200 rounded-xl py-4 px-4',
                        'flex items-center justify-center gap-2 text-sm text-brand-muted',
                        'hover:border-rose hover:text-rose transition-colors cursor-pointer',
                    ...
                    <p className="text-xs text-brand-muted mt-1">
                      First image = main product image. You can upload multiple.
                    </p>
                  </div>
